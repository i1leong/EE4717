...
function retrieve_appointment(){
	include "db.php";
	$pid = $_SESSION['id'];
	$sql = "SELECT * FROM `bookingcalendar` WHERE patient_id=$pid ORDER BY start_epoch DESC";
...
if (mysqli_num_rows($result) > 0){
	...
	while ($row = mysqli_fetch_assoc($result)) {
		...
		$appointment.= '<label class = "appointment_schedule" for="time" id="time">Time: '.sprintf("%02d:%02d",$row["start_hour"],$row["start_minute"]).'-'.sprintf("%02d:%02d",$row["end_hour"],$row["end_minute"]).'</label></td>';
			$appointment.= '<td id="clinic-schedule-25">';
			$appointment.= '<p>Remarks:'.$row['remarks'].'</p>';
			$appointment.= '</td>';
...
else { $appointment = "<p>You do not have any past appointments. <a href='doctor.php'>Make one now</a></p>";}
return $appointment;
}
?>
        </div>
